...
{lib}: let
  ...
  defaultExcludeMatch = ["^[^/.][^/]*$"]; # top-level item not starting with "."

  mkHomeFilesFromDotfiles = {
    dotfiles,
    expandDirs ? [],
    excludeTop ? defaultExcludeTop,
    excludeMatch ? defaultExcludeMatch,
  }: let
    ...
    pref = "${dotRoot}/";
    relFromDot = abs: let
      s = builtins.toString abs;
      r =
        builtins.substring (builtins.stringLength pref)
        (builtins.stringLength s - builtins.stringLength pref)
        s;
    in
    ...
    mkTopItem = name: let
      ...
      src = dotfiles + "/${rel}";
    in
      if ty == "directory" && builtins.elem name expandDirs
      then let
      ...
      in
        map (
          ...
          in {
            name = r;
            value = {source = dotfiles + "/${r}";};
          }
        )
        kept
      else if ty == "directory"
      then
        if matchesAny rel excludeMatch
        then []
        else [
          {
            name = rel;
